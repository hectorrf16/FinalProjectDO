...
openssl req -new -key server.key -out server.csr -subj "/C=ES/ST=CAT/L=BCN/O=HECTOR/OU=HRF/CN=localhost" > /dev/null
openssl req -new -key server.key -out server.csr -nodes -subj "/C=ES/ST=CAT/L=BCN/O=HECTOR/OU=HRF/CN=localhost" > /dev/null
